...
def receive():
    directory = 3
    name, expected_hash, expected_size, *limits = sys.argv[1:]
    ...
    if len(limits) > 1:
        raise ValueError("invalid owned download limit")
    limit = int(limits[0]) if limits else (1024**3 if expected_size is None else expected_size)
    if limit < 1:
        raise ValueError("invalid owned download limit")
